    ...
    public partial class frmEditarFuncionarioCurso : Form
    {
        ...
        private void btnGerarCertificado_Click(object sender, EventArgs e)
        {
            try
            {
                if (mtbConclusao.Text.Length < 10 || mtbConclusao.Text.Contains("_"))
                {
                    MessageBox.Show("O funcionário ainda não possui data de conclusão registrada.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                using (SaveFileDialog sfd = new SaveFileDialog())
                {
                    ...
                    sfd.FileName = "Certificado.docx";

                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        FuncionarioCursoDAO dao = new FuncionarioCursoDAO();
                        var dados = dao.buscarDadosCertificado(_idFuncionarioCurso);

                        CertificadoCursoService service = new CertificadoCursoService();
                        service.GerarCertificado(
                            dados.nomeFuncionario,
                            dados.nomeCurso,
                            dados.descricaoCurso,
                            dados.cargaHoraria,
                            dados.validadeMeses,
                            dados.dataConclusao,
                            sfd.FileName
            ...
            {
                MessageBox.Show("Erro ao gerar certificado: " + ex.Message);
            }
        }
    }
}
